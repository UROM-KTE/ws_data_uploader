...
def test_connect_success(sample_settings, mock_db_logger):
    ...
    with patch('psycopg2.connect') as mock_connect:
        ...
        assert db_manager.connection is mock_connection
        mock_connect.assert_called_once_with(
            host=sample_settings["database_host"],
            port=sample_settings["database_port"],
            dbname=sample_settings["database_name"],
            user=sample_settings["database_user"],
            password=sample_settings["database_password"]
        )
...
def test_connect_failure(sample_settings, mock_db_logger):
    ...
    with patch('psycopg2.connect', side_effect=Exception("Connection error")) as mock_connect:
        db_manager = DatabaseManager(sample_settings)
        result = db_manager.connect()
...
def test_is_connected_no_connection(sample_settings, mock_db_logger):
    ...
    with patch('psycopg2.connect') as mock_connect:
        ...
        assert result is True
        mock_db_logger.debug.assert_any_call("No active connection, attempting to connect")
        mock_connect.assert_called_once()


def test_is_connected_with_connection(sample_settings, mock_db_logger):
    """Test is_connected with an existing connection"""
    db_manager = DatabaseManager(sample_settings)
    ...
    mock_db_logger.debug.assert_called_with("Database connection verified")
...
def test_is_connected_with_error(sample_settings, mock_db_logger):
    ...
    with patch('psycopg2.connect') as mock_connect:
        ...
        assert db_manager.connection is mock_new_connection
...
def test_save_data_connection_error(sample_settings, sample_weather_data, mock_db_logger):
    ...
    with patch.object(DatabaseManager, 'is_connected', return_value=True), \
            patch.object(DatabaseManager, 'connect', return_value=True):
        db_manager.connection = mock_connection
        ...
        mock_db_logger.info.assert_called_once_with("Attempting to reconnect to database")


def test_save_data_no_connection_object(sample_settings, sample_weather_data, mock_db_logger):
    """Test save_data when is_connected is True but the connection object is None"""
    with patch.object(DatabaseManager, 'is_connected', return_value=True):
        db_manager = DatabaseManager(sample_settings)
        db_manager.connection = None
...
def test_connect_with_settings_validation(sample_settings, mock_db_logger):
    ...
    with patch('psycopg2.connect') as mock_connect:
        ...
        mock_db_logger.error.assert_called_once()
